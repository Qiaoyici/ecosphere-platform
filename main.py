...
class PersonalCalcRequest(BaseModel):
    car_km_per_year: float = Field(
        default=0.0, ge=0.0, le=1000000.0, description="Annual driving distance in km"
    )
    flight_hours_per_year: float = Field(
        default=0.0, ge=0.0, le=1000.0, description="Annual flight hours"
    )
    diet_type: Literal[
        "beef_heavy", "high_meat", "poultry_fish", "low_meat",
        "vegetarian", "plant_based_meat", "vegan", "locally_sourced_veg"
    ] = Field(default="vegetarian", description="Diet preference")
    electricity_kwh_per_year: float = Field(
        default=0.0, ge=0.0, le=100000.0, description="Annual electricity usage in kWh"
    )
    waste_kg_per_year: float = Field(
        default=0.0, ge=0.0, le=50000.0, description="Annual waste generated in kg"
    )


class GreenSecCalcRequest(BaseModel):
    crypto_ops_millions: float = Field(
        default=0.0, ge=0.0, le=100000.0, description="Millions of operations"
    )
    crypto_type: Literal["rsa4096", "rsa2048", "ecc256"] = Field(
        default="ecc256", description="Algorithm type"
    )
    log_size_gb: float = Field(
        default=0.0, ge=0.0, le=1000000.0, description="Size of SIEM logging in GB"
    )
    ...
    scan_runs_per_month: float = Field(
        default=0.0, ge=0.0, le=100000.0, description="Security scans run per month"
    )
    ...
    server_hours_per_month: float = Field(
        default=0.0, ge=0.0, le=744000.0, description="Server hosting hours per month"
    )
...
async def calculate_personal(req: PersonalCalcRequest) -> Dict[str, Any]:
    ...
    # Validate and clamp inputs to prevent overflows or injection boundaries
    car_km = security_utils.validate_numeric_input(req.car_km_per_year, 0, 1000000)
    flights = security_utils.validate_numeric_input(req.flight_hours_per_year, 0, 1000)
    diet = security_utils.sanitize_string(req.diet_type)
    electricity = security_utils.validate_numeric_input(req.electricity_kwh_per_year, 0, 100000)
    waste = security_utils.validate_numeric_input(req.waste_kg_per_year, 0, 50000)
    
    # Run math
    emissions = calc_engine.calculate_personal_footprint(car_km, flights, diet, electricity, waste)
    equivalency = calc_engine.get_relatable_equivalencies(emissions["total_kg"])
    
    logger.info(f"calculate_personal endpoint success: total_kg={emissions['total_kg']}")
    return {
        "status": "success",
        "data": {
            "emissions": emissions,
...
async def calculate_greensec(req: GreenSecCalcRequest) -> Dict[str, Any]:
    ...
    """
    logger.info(f"calculate_greensec endpoint called with crypto_type={req.crypto_type}")
    # Sanitize and validate inputs
    crypto_ops = security_utils.validate_numeric_input(req.crypto_ops_millions, 0, 100000)
    crypto_type = security_utils.sanitize_string(req.crypto_type)
    log_size = security_utils.validate_numeric_input(req.log_size_gb, 0, 1000000)
    log_storage = security_utils.sanitize_string(req.log_storage_type)
    scan_runs = security_utils.validate_numeric_input(req.scan_runs_per_month, 0, 100000)
    scan_type = security_utils.sanitize_string(req.scan_type)
    server_hours = security_utils.validate_numeric_input(req.server_hours_per_month, 0, 744000)
    agent_type = security_utils.sanitize_string(req.agent_type)
    
    # Run math
    emissions = calc_engine.calculate_greensec_footprint(
        crypto_ops, crypto_type, log_size, log_storage, scan_runs, scan_type, server_hours, agent_type
    )
    equivalency = calc_engine.get_relatable_equivalencies(emissions["total_kg"])
    
    logger.info(f"calculate_greensec endpoint success: total_kg={emissions['total_kg']}")
    return {
        "status": "success",
        "data": {
            "emissions": emissions,
...
async def verify_state(req: StateVerifyRequest) -> Dict[str, Any]:
    ...
    success, data = security_utils.decrypt_and_verify_state(req.payload, req.signature)
    if not success:
...
async def get_leaderboard() -> List[Dict[str, Any]]:
    ...
    """
    return [
        {"rank": 1, "team": "Kernel Security Core", "score": 98.4, "mode": "GreenSec", "co2_saved_kg": 4820.0},
        {"rank": 2, "team": "Design & UI/UX Guild", "score": 94.2, "mode": "Personal", "co2_saved_kg": 2900.5},
        {"rank": 3, "team": "SecOps Automation", "score": 91.0, "mode": "GreenSec", "co2_saved_kg": 4150.2},
        {"rank": 4, "team": "API Integration Devs", "score": 86.5, "mode": "GreenSec", "co2_saved_kg": 1820.8},
        {"rank": 5, "team": "Product Growth Team", "score": 79.1, "mode": "Personal", "co2_saved_kg": 1200.0}
    ]
